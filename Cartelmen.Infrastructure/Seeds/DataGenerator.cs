...
namespace Cartelmen.Infrastructure.Seeds
{
    public class DataGenerator
    {
        ...
        public async Task Seed()
        {
            ...
            List<Entity.Person> persons;
            if (_dbContext.Person.Any())
            {
                persons = await _dbContext.Person.ToListAsync();
            }
            else
            {
                ...
                persons = personGenerator.Generate(35).ToList();
            ...
            //spotPerson
            List<SpotPerson> spotPersons;
            if (_dbContext.SpotPerson.Any())
            {
                spotPersons = await _dbContext.SpotPerson.ToListAsync();
            }
            else
            {
                ...
                spotPersons = spotPersonGenerator
                    .Generate((int)(spots.Count * persons.Count * 0.6))
                    .DistinctBy(sp => new {sp.SpotId,sp.PersonId})
                    .ToList();
